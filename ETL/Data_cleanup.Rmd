...
human_development<-gather(HD,key="ind_id",value="ind_value",na.rm=TRUE,LE:HE)
names(human_development)<-c("ctry_cd","year","ind_id","ind_value")
```
...
names(substance_abuse)<-c("year","ctry_cd","ind_id","ind_value")
substance_abuse<-substance_abuse[,c("ctry_cd","year","ind_id","ind_value")]
```
...
# Transform to long format
social_determinants<-gather(SD,key="ind_id",value="ind_value",na.rm=TRUE,POPD:HAPS)
names(social_determinants)<-c("ctry_cd","year","ind_id","ind_value")
```
...
names(infect_ctrl_prevent)<-c("year","ctry_cd","ind_id","ind_value")
infect_ctrl_prevent<-infect_ctrl_prevent[,c("ctry_cd","year","ind_id","ind_value")]
```
...
names(disease)<-c("year","ctry_cd","ind_id","ind_value")
disease<-disease[,c("ctry_cd","year","ind_id","ind_value")]
```
...
dbWriteTable(con,"human_development",human_development,overwrite=TRUE)
dbWriteTable(con,"substance_abuse",substance_abuse,overwrite=TRUE)
dbWriteTable(con,"social_determinants",social_determinants,overwrite=TRUE)
dbWriteTable(con,"infect_ctrl_prevent",infect_ctrl_prevent,overwrite=TRUE)
dbWriteTable(con,"disease",disease,overwrite=TRUE)
dbWriteTable(con,"country",country,overwrite=TRUE)

```


```{r Getting data from database}
data<-dbGetQuery(con,'SELECT c.ctry_cd AS Country_Code,
                  	   c.ctry_desc AS Country,
                  	   c.status AS Status,
                  	   i.year AS Year,
                  	   i.ind_id AS Indicator,
