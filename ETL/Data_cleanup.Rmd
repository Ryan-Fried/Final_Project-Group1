...
# Handling missing values
income<-income%>%group_by(Country)%>%filter(!mean(is.na(INCOME)) >= 0.8)
...
# Final cleanup
income%>%group_by(Country)%>%miss_var_summary()%>%filter(n_miss>0)
names(income)<-c("Country","Year","Income")
```
...
# Handling missing values
edu<-edu%>%group_by(Country)%>%filter(!mean(is.na(EDU)) >= 0.8)
...
# Final cleanup
edu%>%group_by(Country)%>%miss_var_summary()%>%filter(n_miss>0)
names(edu)<-c("Country","Year","Education")
```
